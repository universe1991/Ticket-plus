...
public class MySQLConnection implements DBConnection{
	...
	public void setFavoriteItems(String userName, List<String> itemIds) {
		...
		try {
			String sql = "INSERT IGNORE INTO history (user_name, item_id) VALUES (?, ?)";
	...
	public void unsetFavoriteItems(String userName, List<String> itemIds) {
		...
		try {
			String sql = "DELETE FROM history WHERE user_name = ? AND item_id = ?";
			PreparedStatement stmt = conn.prepareStatement(sql);
			for (String itemId : itemIds) {
				stmt.setString(1, userName);
				stmt.setString(2, itemId);
	...
	public Set<String> getFavoriteItemIds(String userName) {
		...
		try {
			String sql = "SELECT item_id from history where user_name = ?";
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, userName);
	...
	public String getFullname(String userName) {
		...
		try {
			...
			ResultSet rs = statement.executeQuery();
			if (rs.next()) {
				name = String.join(" ", rs.getString("user_name"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String registerNewUser(String userName, String passWord) {
		...
		try {
			...
			if (rs.next()) {
				...
				String sql1 = "INSERT IGNORE users(user_name, password) VALUES (?, ?)"; 
